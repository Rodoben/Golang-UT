...
func TestForm_Has(t *testing.T) {
	...
	if has {
		t.Error("form shows has field when it should not", has)
	}
...
func TestForm_Required(t *testing.T) {
	...
	if form.Valid() {
		t.Error("form shows valid when required fields are mssing")
	}
	...
	form.Required("a", "b", "c")
	if !form.Valid() {
		t.Error("shows post does not have required fie.ds, when it does")
	}
}
